...
class SentientBrain {
    constructor(connection, openaiApiKey, sniperEngine, walletPublicKey) {
    ...
    async loadState() {
        try {
            ...
            const parsed = JSON.parse(data, (key, value) => {
                if (value && value._type === "PublicKey")
                    return new web3_js_1.PublicKey(value.value);
                if (value && value._type === "BN")
                    return new bn_js_1.default(value.value, 16);
                return value;
            });
        ...
        try {
            ...
            const json = JSON.stringify(state, (key, value) => {
                if (value && value.toBase58)
                    return { _type: "PublicKey", value: value.toBase58() };
                if (value && bn_js_1.default.isBN(value))
                    return { _type: "BN", value: value.toString(16) };
                return value;
            }, 2);
    ...
    async analyzeToken(mint, tokenData) {
        const MAX_RETRIES = 3;
        let attempt = 0;
...
Rate it 1-10 for humor/virality/potential to pump. Return ONLY valid JSON: {score: <1-10>, reason: "<2 sentence reason>"}`;
                ...
                const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("OpenAI Timeout")), 15000));
                ...
                try {
                    ...
                    console.log(`🧠 AI Analysis for ${tokenData.name}: Score ${score}/10`);
    ...
    async recordPosition(mint, aiScore, analysis, poolKeys) {
        let finalEntryPrice = 1.0;
        let finalAmount = 0;
        // 1. Fetch Real Balance (wait for bundle fill)
        try {
            const { amountRaw } = await this.fetchTokenBalanceWithRetry(mint, 30, 500, true);
            if (amountRaw > BigInt(0)) {
                finalAmount = this.bigIntToSafeNumber(amountRaw);
                console.log(`   └─ Confirmed on-chain balance: ${finalAmount}`);
            }
            else {
                console.warn(`   └─ No tokens detected for ${mint} after waiting. Skipping position.`);
                return;
            }
        }
        catch (e) {
        ...
        if (poolKeys) {
            try {
                const price = await (0, raydium_1.getPoolPrice)(this.connection, poolKeys);
                if (price > 0) {
        ...
        this.activePositions.set(mint, position);
        this.saveState();
        console.log(`📊 Position recorded: ${mint} | Score: ${aiScore}/10 | Entry: $${finalEntryPrice.toFixed(6)}`);
        // Determine exit strategy based on score
        if (aiScore < config_1.AI_SCORE_IMMEDIATE_SELL) {
    ...
    monitorPosition(mint) {
        ...
        const interval = setInterval(async () => {
            ...
            const takeProfit = position.aiScore > config_1.AI_SCORE_HOLD_LONG
                ? config_1.TAKE_PROFIT_HIGH
                : config_1.TAKE_PROFIT_LOW;
            const stopLoss = position.aiScore > config_1.AI_SCORE_HOLD_LONG ? config_1.STOP_LOSS_HIGH : config_1.STOP_LOSS_LOW;
            // Short log every 10 seconds
            if (Math.random() < 0.1) {
                console.log(`📈 ${mint}: ${percentChange.toFixed(2)}% | TP: ${(takeProfit * 100).toFixed(0)}% | SL: ${(stopLoss * 100).toFixed(0)}%`);
            }
            // Check if TP or SL hit
            if (priceChange >= takeProfit) {
                console.log(`🚀 TAKE PROFIT HIT: ${mint} | Gain: +${percentChange.toFixed(2)}%`);
                // Determine if we want to moonbag
                let sellAmount = position.amount;
                const estValueSol = (position.amount / Math.pow(10, config_1.DEFAULT_TOKEN_DECIMALS)) *
                    currentPrice;
                if (estValueSol >= config_1.MOONBAG_THRESHOLD_SOL) {
                    sellAmount = Math.floor(position.amount * config_1.MOONBAG_SELL_PCT);
                    console.log(`   └─ Moonbag mode: selling ${config_1.MOONBAG_SELL_PCT * 100}% (~${sellAmount} units)`);
                }
                ...
                console.log(`🛑 STOP LOSS HIT: ${mint} | Loss: ${percentChange.toFixed(2)}%`);
                position.status = "closed";
                clearInterval(interval);
                this.monitoringIntervals.delete(mint);
                await this.sell(mint, "Stop loss");
            }
        }, config_1.PRICE_POLL_INTERVAL);
        this.monitoringIntervals.set(mint, interval);
    }
    async getPriceFromRpc(mint) {
        try {
            const position = this.activePositions.get(mint);
            // Use local Raydium calculation (FAST & REAL)
            if (position?.poolKeys) {
                return await (0, raydium_1.getPoolPrice)(this.connection, position.poolKeys);
            }
    ...
    async sell(mint, reason = "Manual", isEmergency = false, overrideAmount) {
        try {
            const position = this.activePositions.get(mint);
            if (!position) {
            ...
            let amountRaw = BigInt(overrideAmount ?? position.amount);
            try {
                const balance = await this.fetchTokenBalanceWithRetry(mint, isEmergency ? 1 : 5);
            ...
            const bundleId = await this.sniperEngine.sell(mint, amountNumber, isEmergency);
            if (bundleId) {
     ...
     */
    getActivePositions() {
        return Array.from(this.activePositions.values()).filter((p) => p.status === "open");
    }
    /**
    ...
    async fetchTokenBalanceWithRetry(mint, retries = 5, delayMs = 500, requirePositive = false) {
        const ata = await (0, spl_token_1.getAssociatedTokenAddress)(new web3_js_1.PublicKey(mint), this.walletPublicKey);
    ...
    bigIntToSafeNumber(value) {
        const max = BigInt(Number.MAX_SAFE_INTEGER);
        if (value > max) {
            console.warn(`Value ${value.toString()} exceeds MAX_SAFE_INTEGER. Truncating.`);
            return Number(max);
        }
        return Number(value);
    }
}
exports.SentientBrain = SentientBrain;
//# sourceMappingURL=SentientBrain.js.map