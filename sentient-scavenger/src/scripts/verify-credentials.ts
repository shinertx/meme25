...
async function verify() {
  ...
  try {
    ...
    if (!rpcUrl) throw new Error("SOLANA_RPC_URL missing in .env");
  ...
  try {
    ...
    if (!pkFormat) throw new Error("SOLANA_PRIVATE_KEY missing in .env");
    ...
    if (pkFormat.includes("[")) {
      ...
      keypair = Keypair.fromSecretKey(bs58.decode(pkFormat));
    }
  ...
  try {
    ...
    await axios.get(jitoUrl).catch(e => {
        if (e.response) {
