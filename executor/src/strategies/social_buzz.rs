...
impl Strategy for SocialBuzz {
    ...
    fn subscriptions(&self) -> HashSet<EventType> {
        [
            EventType::Social,
            EventType::TwitterRaw,
            EventType::FarcasterRaw,
        ]
        .iter()
        .cloned()
        .collect()
    ...
    async fn on_event(&mut self, event: &MarketEvent) -> Result<StrategyAction> {
        match event {
            MarketEvent::Social(mention) => {
                ...
                let recent_mentions: Vec<u32> =
                    history.iter().rev().take(3).map(|d| d.mentions).collect();

                let historical_mentions: Vec<u32> =
                    history.iter().skip(3).map(|d| d.mentions).collect();
                ...
                let recent_avg =
                    recent_mentions.iter().sum::<u32>() as f64 / recent_mentions.len() as f64;
                let historical_avg = historical_mentions.iter().sum::<u32>() as f64
                    / historical_mentions.len() as f64;
                    ...
                    && latest.sentiment > 0.5
                {
                    ...
                    self.last_trade
                        .insert(token_addr.clone(), mention_timestamp);
                    ...
                    let order = OrderDetails {
                        ...
                        strategy_metadata: HashMap::from([
                            ...
                            (
                                ...
                                serde_json::json!(latest_engagement),
                            ),
                            (
                                "influencer_score".to_string(),
                                serde_json::json!(latest_influencer_score),
                            ),
                        ]),
            ...
            MarketEvent::TwitterRaw(tweet) => {
                debug!(
                    ...
                    "Processed Twitter raw event for sentiment cache"
                );
            }
            MarketEvent::FarcasterRaw(cast) => {
                debug!(
                    cast_hash = %cast.cast_hash,
                    followers = cast.author_followers,
                    "Processed Farcaster raw event for sentiment cache"
                );
            }
            _ => {}
        }
