...
impl Strategy for BridgeInflow {
    ...
    fn subscriptions(&self) -> HashSet<EventType> {
        [EventType::Bridge, EventType::OnChain]
            .iter()
            .cloned()
            .collect()
    }
    ...
    async fn on_event(&mut self, event: &MarketEvent) -> Result<StrategyAction> {
        match event {
            MarketEvent::Bridge(bridge_event) => self.process_bridge_flow(bridge_event).await,
...
impl BridgeInflow {
    async fn process_bridge_flow(
        &mut self,
        bridge_event: &shared_models::BridgeEvent,
    ) -> Result<StrategyAction> {
        debug!(
            "Processing bridge flow: {} USD from {} to {}",
            bridge_event.volume_usd, bridge_event.source_chain, bridge_event.destination_chain
        );
        ...
        self.evaluate_bridge_signal(&bridge_event.token_address, &flow)
            .await
    }
    ...
    fn calculate_velocity_score(&self, amount_usd: f64) -> f64 {
        ...
        let size_multiplier = if amount_usd > 1_000_000.0 {
            2.5
        } else if amount_usd > 500_000.0 {
            1.8
        } else if amount_usd > 250_000.0 {
            1.3
        } else {
            1.0
        };
    ...
    async fn evaluate_bridge_signal(
        &mut self,
        token_address: &str,
        flow: &BridgeFlow,
    ) -> Result<StrategyAction> {
        ...
        if self.active_positions >= 3 {
            debug!(
                "Bridge strategy at position limit ({})",
        ...
        if flow.amount_usd >= self.min_inflow_usd && flow.velocity_score >= self.velocity_threshold
        {
            ...
            let recent_total: f64 = flows
                .iter()
                .filter(|f| {
                    flow.timestamp
                        .signed_duration_since(f.timestamp)
                        .num_hours()
                        < 2
                })
                .map(|f| f.amount_usd)
            ...
            if confidence > 0.6 {
                ...
                self.recent_signals
                    .insert(token_address.to_string(), flow.timestamp);
                ...
                let order = OrderDetails {
                    ...
                    strategy_metadata: HashMap::from([
                        (
                            ...
                            serde_json::json!(flow.amount_usd),
                        ),
                        (
                            "velocity_score".to_string(),
                            serde_json::json!(flow.velocity_score),
                        ),
    ...
    async fn process_onchain_follow_up(
        &mut self,
        chain_event: &shared_models::OnChainEvent,
    ) -> Result<StrategyAction> {
        ...
        debug!(
            ...
            chain_event.token_address, chain_event.event_type
        );
        ...
        if chain_event.event_type == "swap" || chain_event.event_type == "liquidity_add" {
            ...
            if let Some(_flows) = self.bridge_flows.get(&chain_event.token_address) {
                ...
                debug!(
                    "On-chain activity detected after bridge flow for {}",
