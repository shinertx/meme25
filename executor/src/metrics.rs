...
pub struct Metrics {
    ...
    pub risk_events_by_strategy: CounterVec,
    ...
    pub exposure_utilization_pct: Gauge,
    registry: Registry,
}
...
impl Metrics {
    pub fn new(metrics_port: Option<u16>) -> Result<Arc<Self>> {
        ...
        let trades_total =
            Counter::with_opts(Opts::new("trades_total", "Total number of trades executed"))
                .map_err(|e| {
                    ModelError::Config(format!("Failed to create trades_total metric: {}", e))
                })?;
        ...
        .map_err(|e| {
            ModelError::Config(format!("Failed to create portfolio_value metric: {}", e))
        })?;

        let daily_pnl =
            Gauge::with_opts(Opts::new("daily_pnl_usd", "Daily profit and loss in USD"))?;
        ...
        let risk_events_by_strategy = CounterVec::new(
            Opts::new(
                "risk_events_by_strategy",
                "Risk events grouped by strategy and reason",
            ),
            &["strategy", "reason"],
        )?;
        ...
        let slippage_histogram = Histogram::with_opts(
            HistogramOpts::new("slippage_basis_points", "Slippage in basis points").buckets(vec![
                1.0, 5.0, 10.0, 25.0, 50.0, 100.0, 250.0, 500.0, 1000.0,
            ]),
        )?;

        let jupiter_quote_time = Histogram::with_opts(
            HistogramOpts::new(
        ...
        let mev_protection_time = Histogram::with_opts(
            HistogramOpts::new(
        ...
        let exposure_utilization_pct = Gauge::with_opts(Opts::new(
            "portfolio_exposure_utilization_pct",
            "Current portfolio exposure utilization in percent",
        ))?;
        ...
        registry
            ...
            .map_err(|e| {
                ModelError::Config(format!("Failed to register portfolio_value: {}", e))
            })?;
        registry
            .register(Box::new(daily_pnl.clone()))
            .map_err(|e| ModelError::Config(format!("Failed to register daily_pnl: {}", e)))?;
        registry
            .register(Box::new(strategy_performance.clone()))
            .map_err(|e| {
                ModelError::Config(format!("Failed to register strategy_performance: {}", e))
            })?;
        registry
            .register(Box::new(risk_events.clone()))
            .map_err(|e| ModelError::Config(format!("Failed to register risk_events: {}", e)))?;
        registry
            .register(Box::new(risk_events_by_strategy.clone()))
            .map_err(|e| {
                ModelError::Config(format!("Failed to register risk_events_by_strategy: {}", e))
            })?;
        registry
        ...
        registry
            .register(Box::new(slippage_histogram.clone()))
            .map_err(|e| {
                ModelError::Config(format!("Failed to register slippage_histogram: {}", e))
            })?;
        registry
            .register(Box::new(jupiter_quote_time.clone()))
            .map_err(|e| {
                ModelError::Config(format!("Failed to register jupiter_quote_time: {}", e))
            })?;
        registry
            .register(Box::new(mev_protection_time.clone()))
            .map_err(|e| {
                ModelError::Config(format!("Failed to register mev_protection_time: {}", e))
            })?;
        registry
            .register(Box::new(data_freshness.clone()))
            .map_err(|e| ModelError::Config(format!("Failed to register data_freshness: {}", e)))?;
        registry
            .register(Box::new(exposure_utilization_pct.clone()))
            .map_err(|e| {
                ModelError::Config(format!(
                    "Failed to register exposure_utilization_pct: {}",
                    e
                ))
            })?;
    ...
    pub fn record_risk_event(&self, strategy: &str, reason: &str) {
        let strategy_label = if strategy.is_empty() {
            "unattributed"
        } else {
            strategy
        };
        ...
        self.risk_events_by_strategy
            .with_label_values(&[strategy_label, reason])
            .inc();
    }
    ...
    pub fn update_exposure_utilization(&self, utilization_pct: f64) {
        let clamped = utilization_pct.clamp(0.0, 500.0);
        self.exposure_utilization_pct.set(clamped);

        if utilization_pct > 95.0 {
            warn!(
                "Portfolio exposure utilization climbing: {:.2}%",
                utilization_pct
            );
        }
    }
...
pub fn spawn_metrics_server(metrics: Arc<Metrics>, port: u16) {
    tokio::spawn(async move {
        if let Err(e) = start_metrics_server(port, metrics).await {
