...
impl PortfolioAllocator {
    pub fn new() -> Self {
        ...
        for strategy_id in strategy_ids {
            strategy_weights.insert(strategy_id.to_string(), StrategyWeight {
    ...
    pub fn update_strategy_performance(&mut self, strategy_id: &str, performance: PerformanceMetric) -> Result<()> {
        // Add to performance history
        let history = self.performance_history.entry(strategy_id.to_string()).or_insert_with(Vec::new);
    ...
    fn calculate_strategy_scores(&mut self, strategy_id: &str) -> Result<()> {
        ...
        let weight = self.strategy_weights.get_mut(strategy_id)
        ...
        weight.sharpe_ratio = if volatility > 0.0 { avg_return / volatility } else { 0.0 };
        ...
        let avg_trades = history.iter().map(|p| p.trades_count as f64).sum::<f64>() / history.len() as f64;
        ...
        weight.performance_score = (0.4 * sharpe_component + 
                                   ...
                                   0.1 * drawdown_penalty).clamp(0.0, 1.0);
    ...
    pub fn rebalance_portfolio(&mut self) -> Result<()> {
        ...
        let total_performance: f64 = self.strategy_weights.values()
    ...
    fn calculate_regime_bonus(&self, strategy_id: &str) -> f64 {
        match (&self.market_regime, strategy_id) {
            ...
            (MarketRegime::Volatile, id) if id.contains("rug_pull") || id.contains("circuit") => 0.15,
    ...
    fn update_market_regime(&mut self) {
        ...
        let momentum_performance: f64 = self.strategy_weights.values()
            .filter(|w| w.strategy_id.contains("momentum"))
            .map(|w| w.performance_score)
            .sum();
            
        let reversion_performance: f64 = self.strategy_weights.values()
        ...
        if new_regime != self.market_regime {
            info!("📈 Market regime change detected: {:?} → {:?}", self.market_regime, new_regime);
    ...
    fn apply_allocation_constraints(&mut self) -> Result<()> {
        for (_, weight) in &mut self.strategy_weights {
            if weight.is_active {
                weight.target_weight = weight.target_weight
    ...
    fn normalize_weights(&mut self) {
        let total_weight: f64 = self.strategy_weights.values()
            .filter(|w| w.is_active)
    ...
    fn apply_correlation_adjustments(&mut self) {
        ...
        for (strategy_id, weight) in &self.strategy_weights {
            if weight.is_active {
                let similar_weight: f64 = self.strategy_weights.values()
                    .filter(|w| w.is_active && self.are_strategies_similar(strategy_id, &w.strategy_id))
                    .map(|w| w.target_weight)
                    .sum();
        ...
        for (strategy_id, weight) in &mut self.strategy_weights {
            ...
            if let Some(&similar_weight) = strategy_similar_weights.get(strategy_id) {
                if similar_weight > 0.3 { // If similar strategies exceed 30%
    ...
    fn are_strategies_similar(&self, strategy1: &str, strategy2: &str) -> bool {
        ...
        (mean_reversion_strategies.contains(&strategy1) && mean_reversion_strategies.contains(&strategy2)) ||
    ...
    fn set_equal_weights(&mut self) -> Result<()> {
        let active_count = self.strategy_weights.values().filter(|w| w.is_active).count();
        if active_count == 0 {
            return Err(ModelError::Strategy("No active strategies available".into()));
        }
    ...
    fn validate_allocations(&self) -> Result<()> {
        ...
        if (total_weight - 1.0).abs() > 0.01 {
            return Err(ModelError::Strategy(format!("Invalid weight sum: {:.3}", total_weight)));
        }
        
        for (id, weight) in &self.strategy_weights {
            if weight.is_active && (weight.target_weight < 0.0 || weight.target_weight > self.max_single_allocation * 1.1) {
                return Err(ModelError::Strategy(format!("Invalid allocation for {}: {:.3}", id, weight.target_weight)));
            }
        }
    ...
    fn log_rebalance_results(&self) {
        info!("🎯 Portfolio rebalancing completed - Market regime: {:?}", self.market_regime);
        
        let mut sorted_weights: Vec<_> = self.strategy_weights.values()
            .filter(|w| w.is_active)
            .collect();
    ...
    pub fn set_strategy_active(&mut self, strategy_id: &str, active: bool) -> Result<()> {
        match self.strategy_weights.get_mut(strategy_id) {
            Some(weight) => {
                ...
                info!("Strategy {} set to {}", strategy_id, if active { "active" } else { "inactive" });
                Ok(())
            }
            None => Err(ModelError::Strategy(format!("Unknown strategy: {}", strategy_id)))
        }
    }

    /// Force immediate rebalancing
    pub fn force_rebalance(&mut self) -> Result<()> {
        self.last_rebalance = Utc::now() - Duration::hours(self.rebalance_frequency_hours as i64 + 1);
        self.rebalance_portfolio()
    }
}
