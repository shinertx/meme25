...
impl CorrelationManager {
    ...
    pub fn record_strategy_return(&mut self, strategy_return: StrategyReturn) -> Result<()> {
        ...
        let returns = self.strategy_returns.entry(strategy_id.clone()).or_insert_with(Vec::new);
        ...
        debug!("Recorded return for {}: {:.2}%", strategy_id, returns.last().unwrap().return_pct);
    ...
    pub fn calculate_correlations(&mut self) -> Result<Vec<CorrelationAlert>> {
        ...
        for i in 0..strategy_ids.len() {
            for j in (i + 1)..strategy_ids.len() {
                ...
                if let Some(correlation) = self.calculate_pairwise_correlation(strategy_a, strategy_b)? {
                    self.correlation_matrix.insert((strategy_a.clone(), strategy_b.clone()), correlation);
                    self.correlation_matrix.insert((strategy_b.clone(), strategy_a.clone()), correlation);
                    ...
                    if correlation.abs() > self.high_correlation_threshold {
                        ...
                        alerts.push(CorrelationAlert {
                            ...
                            recommended_action: if correlation > 0.0 {
                                "Consider reducing position sizes or diversifying timing".to_string()
                            } else {
                                "Negative correlation detected - potential hedging opportunity".to_string()
                            },
                            severity,
                        });
                    }
        ...
        info!("Calculated {} correlations, generated {} alerts", 
              self.correlation_matrix.len() / 2, alerts.len());
        
        Ok(alerts)
    }

    /// Calculate correlation between two strategies
    fn calculate_pairwise_correlation(&self, strategy_a: &str, strategy_b: &str) -> Result<Option<f64>> {
        ...
        match (returns_a, returns_b) {
            (Some(ra), Some(rb)) if ra.len() >= 10 && rb.len() >= 10 => {
                ...
                for i in 0..min_len {
                    if let (Some(ret_a), Some(ret_b)) = (ra.get(ra.len() - min_len + i), rb.get(rb.len() - min_len + i)) {
    ...
    fn pearson_correlation(&self, x: &[f64], y: &[f64]) -> Result<f64> {
        if x.len() != y.len() || x.is_empty() {
            return Err(ModelError::Strategy("Invalid correlation input data".into()));
        }
    ...
    fn identify_correlation_clusters(&mut self) -> Result<()> {
        ...
        for i in 0..strategy_ids.len() {
            ...
            for j in (i + 1)..strategy_ids.len() {
                ...
                let correlation = self.get_correlation(&strategy_ids[i], &strategy_ids[j])
                    .unwrap_or(0.0);
        ...
        debug!("Identified {} correlation clusters", self.correlation_clusters.len());
        Ok(())
    }
    ...
    pub fn update_cluster_allocations(&mut self, strategy_allocations: &HashMap<String, f64>) -> Result<()> {
        for cluster in &mut self.correlation_clusters {
            cluster.total_allocation = cluster.strategies.iter()
                .map(|strategy_id| strategy_allocations.get(strategy_id).copied().unwrap_or(0.0))
                .sum();
    ...
    pub fn get_correlation(&self, strategy_a: &str, strategy_b: &str) -> Option<f64> {
        self.correlation_matrix.get(&(strategy_a.to_string(), strategy_b.to_string())).copied()
    }

    /// Get position adjustment factor for strategy based on correlations
    pub fn get_position_adjustment(&self, strategy_id: &str) -> f64 {
        self.position_adjustments.get(strategy_id).copied().unwrap_or(1.0)
    }

    /// Calculate position adjustments based on correlation analysis
    pub fn calculate_position_adjustments(&mut self, strategy_allocations: &HashMap<String, f64>) -> Result<()> {
        ...
        for strategy_id in strategy_allocations.keys() {
            ...
            let high_corr_count = self.correlation_matrix.iter()
                .filter(|((a, _), &corr)| a == strategy_id && corr.abs() > self.high_correlation_threshold)
                .count();
            ...
            self.position_adjustments.insert(strategy_id.clone(), adjustment_factor);
        }
    ...
    pub fn force_recalculation(&mut self) -> Result<Vec<CorrelationAlert>> {
        self.last_calculation = Utc::now() - Duration::hours(self.calculation_frequency_hours as i64 + 1);
        self.calculate_correlations()
    }
    ...
    pub fn get_correlation_summary(&self) -> HashMap<String, serde_json::Value> {
        ...
        summary.insert("total_correlations".to_string(), 
                      serde_json::json!(self.correlation_matrix.len() / 2));
        summary.insert("high_correlations".to_string(),
                      serde_json::json!(self.correlation_matrix.values()
                          .filter(|&&corr| corr.abs() > self.high_correlation_threshold).count() / 2));
        summary.insert("clusters".to_string(),
                      serde_json::json!(self.correlation_clusters.len()));
        summary.insert("last_calculation".to_string(),
